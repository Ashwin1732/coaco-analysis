...
function populateResultBlock(mapped, blockNode, fileObj) {
  ...
  blockNode.id = idStr;
  blockNode.querySelector(".sample-heading").textContent =
    `Sample ${parseInt(mapped.sample_id) + 1}`;
  ...
  function updatePlayback(elapsed) {
    timeEl.textContent =
      formatTime(elapsed) + " / " + formatTime(duration || 0);
  ...
  if (mapped.confidence < CONFIDENCE_THRESHOLD) {
    ...
    listEl.innerHTML = mapped.other_possibilities
      .map((item) => {
...
uploadForm.addEventListener("submit", async (e) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!tFile || !aFile) {
      setStatus(
        `Sample ${i + 1} is incomplete. Please provide both files.`,
        true,
      );
      return;
    }
    ...
    formData.append(
      row.querySelector('input[name*="_thermal_image"]').name,
      tFile,
    );
    formData.append(
      row.querySelector('input[name*="_acoustic_file"]').name,
      aFile,
    );
    completeRows++;
  }
  ...
  try {
    ...
    data.results.forEach((apiData, i) => {
      ...
      const row = batchSamplesContainer
        .querySelector(`input[name="sample_${mapped.sample_id}_thermal_image"]`)
        .closest(".batch-sample-row");
      const fileObj = row.querySelector('input[name*="_thermal_image"]')
        .files[0];
      ...
      setTimeout(() => {
        const addedBlock = document.getElementById(
          `result-sample-${mapped.sample_id}`,
        );
        if (addedBlock) {
